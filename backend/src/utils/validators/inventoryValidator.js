const validateInventoryData = (mode) => (req, res, next) => {
  ...
  if (mode === 'update') {
    ...
    if (data.min_stock_threshold !== undefined && (!Number.isInteger(data.min_stock_threshold) || data.min_stock_threshold < 0)) {
      errors.min_stock_threshold = 'Minimum stock threshold must be a non-negative integer';
    }

    if (data.max_stock_threshold !== undefined && (!Number.isInteger(data.max_stock_threshold) || data.max_stock_threshold < 0)) {
      errors.max_stock_threshold = 'Maximum stock threshold must be a non-negative integer';
    }
  }
